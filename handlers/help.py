...
async def show_help(message: Message, state: FSMContext, bot: Bot):
    ...
    help_text = (
        ...
        "📖 Учить слова: просматривайте карточки со словами\n" +
        "🎮 Игры: играйте в викторины для закрепления знаний\n" +
        "📝 Тест знаний: пройдите тест из нескольких вопросов\n" +
        "📊 Статистика: посмотрите свои успехи и статистику\n" +
        "❓ Справка: краткая информация о боте и его функциях\n" +
        "🔁 Словари: изменение списка изучаемых слов\n" +
        "⬆️ В главное меню: для возврата в главное меню\n\n" +
        "/start - если не отображаются кнопки (перезапуск бота)\n\n" +
        "<b>📚 Управление словами:</b>\n" +
        f"/my_set - управлять своим личным словарём (создать, добавить, удалить, просмотреть). <b>Максимум {MAX_USER_WORDS} слов</b>\n" +
        f"/msg_to_admin - написать сообщение администратору\n\n" +
        "Для получения этой справки используйте команду /help\n"
    )
    ...
    if message.from_user.id in ADMIN_IDS:
        admin_commands = (
            ...
            "<b>📁 Управление файлами словарей:</b>\n" +
            ...
            f"/deduplicate_words - удалить дубликаты слов во всех файлах словарей\n" +
            "\n" +
            ...
            f"/move_audio_files - перемещение аудиофайлов в `data/sounds`\n" +
            f"/convert_all_audio - конвертация аудиофайлов в mp3\n" +
            f"/delete_audio_files - удалить аудиофайлы из папок\n" +
            f"/reset_all_stats - сбросить всю статистику пользователей\n" +
            f"/send_content <code>[ID_класса]</code> - оправить любой контент пользователям\n" +
            f"/send_msg <code>[текст]</code> или <code>class=ID_класса [текст]</code> - отправить сообщение пользователям\n\n" +
